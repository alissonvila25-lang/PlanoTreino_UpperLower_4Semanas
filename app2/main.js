...
const els = {
  ...
  summaryImportCsv: document.getElementById('summary-import-csv'),
...
function importDataFromCSVText(text){
  try {
    ...
    for (const row of rows){
      const dia = getCaseInsensitive(row, 'Dia') || getCaseInsensitive(row,'day') || getCaseInsensitive(row,'dia');
      const exercicio = getCaseInsensitive(row, 'Exercicio') || getCaseInsensitive(row,'exercise') || getCaseInsensitive(row,'exercicio');
      const semanaRaw = getCaseInsensitive(row, 'Semana') || getCaseInsensitive(row,'week') || getCaseInsensitive(row,'semana');
      if (dia && exercicio && semanaRaw){
        const semana = parseInt(String(semanaRaw),10) || 0;
        if (semana>=1 && semana<=4){
          const id = `${sanitize(dia)}|${sanitize(exercicio)}`;
          const carga = getCaseInsensitive(row, 'Carga');
          const reps = getCaseInsensitive(row, 'Reps');
          const nota = getCaseInsensitive(row, 'Nota');
          const done = getCaseInsensitive(row, 'Done');
          const pr = getCaseInsensitive(row, 'PR');
          if (carga != null){ setEntry(id, semana, 'carga', String(carga)); count++; }
          if (reps != null){ setEntry(id, semana, 'reps', String(reps)); count++; }
          if (nota != null){ setEntry(id, semana, 'nota', String(nota)); count++; }
          if (done != null){ setEntry(id, semana, 'done', (done===true || String(done)==='1') ? '1' : '0'); count++; }
          if (pr != null){ setEntry(id, semana, 'pr', (pr===true || String(pr)==='1') ? '1' : '0'); count++; }
        }
        continue;
      }
      // Formato multi-semana: Carga_S1, Reps_S1...
      if (dia && exercicio){
        const id = `${sanitize(dia)}|${sanitize(exercicio)}`;
        for (let w=1; w<=4; w++){
          const carga = getCaseInsensitive(row, `Carga_S${w}`) || getCaseInsensitive(row, `carga_s${w}`);
          ...
          const nota = getCaseInsensitive(row, `Nota_S${w}`) || getCaseInsensitive(row, `nota_s${w}`) || getCaseInsensitive(row, `nota`);
          const done = getCaseInsensitive(row, `Done_S${w}`) || getCaseInsensitive(row, `done_s${w}`) || getCaseInsensitive(row, `done`);
          const pr = getCaseInsensitive(row, `PR_S${w}`) || getCaseInsensitive(row, `pr_s${w}`) || getCaseInsensitive(row, `pr`);
          if (carga != null){ setEntry(id, w, 'carga', String(carga)); count++; }
          if (reps != null){ setEntry(id, w, 'reps', String(reps)); count++; }
          if (nota != null){ setEntry(id, w, 'nota', String(nota)); count++; }
          if (done != null){ setEntry(id, w, 'done', (done===true || String(done)==='1') ? '1' : '0'); count++; }
          if (pr != null){ setEntry(id, w, 'pr', (pr===true || String(pr)==='1') ? '1' : '0'); count++; }
        }
        continue;
      }
    }
    alert(`Dados importados via CSV:\n${count} entradas.`);
    render();
  } catch (e){ alert('Falha ao importar CSV: ' + e.message); }
}

function importDataFromCSVFile(file){
  const reader = new FileReader();
  reader.onload = () => { importDataFromCSVText(reader.result); };
  reader.readAsText(file);
}
...
if (els.summaryImportCsv) els.summaryImportCsv.addEventListener('click', ()=>{
  const input = document.createElement('input'); input.type = 'file'; input.accept = '.csv,text/csv';
  input.addEventListener('change', ()=>{ if (input.files && input.files[0]) importDataFromCSVFile(input.files[0]); });
  input.click();
});
...
function renderResumo(){
  ...
  Object.keys(byDay).forEach(day => {
    ...
    for (const { ex, e } of items){
      ...
      const parts = [];
      parts.push(`<span>${sanitize(ex.Exercicio)} ${pr ? '<span class=\"badge\">PR</span>' : ''} ${pr ? prRepsBadge : ''}</span>`);
